...
const generateAccessAndRefreshToken = async (staffId) => {
  try {
    ...
    const accessToken = await staff.generateAccessToken();
    const refreshToken = await staff.generateRefreshToken();

    staff.refreshToken = refreshToken; // Updated to use refreshToken instead of cleanRefreshToken
...
const loginStaff = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .cookie("staffAccessToken", accessToken, options)
    .cookie("staffRefreshToken", refreshToken, options)
    .json({
      success: true,
...
const updateStudent = asyncHandler(async (req, res, next) => {
  ...
  // Validate required fields for update (registrationNumber is not required)
  if (
    !_id ||
    !fullName ||
    !email ||
  ...
  try {
    ...
    if (uploadAvatar?.url) {
      try {
        await deleteFromCloudinary(uploadAvatar.url);
      } catch (deleteError) {
...
const getCourses = asyncHandler(async (req, res, next) => {
  ...
  return res.status(200).json({
    success: true,
    data: courses, // Updated to return courses directly as data
...
const getNotification = asyncHandler(async (req, res, next) => {
  ...
  const notifications = await Notification.find({
    $or: [
      { sentTo: staffId, deletedByRecipient: false },
      { sentBy: staffId, deletedBySender: false },
    ],
  }).sort({ createdAt: -1 });
...
const deleteNotification = asyncHandler(async (req, res, next) => {
  ...
  // Check if the staff is either the sender or recipient
  const isSender = notification.sentBy.toString() === staffId.toString();
  const isRecipient = notification.sentTo.toString() === staffId.toString();
...
const clearNotifications = asyncHandler(async (req, res, next) => {
  ...
  // Update notifications where staff is the sender
  ...
  // Update notifications where staff is the recipient
...
const getAdmins = asyncHandler(async (req, res, next) => {
  const admins = await Admin.find({}).select("-password -__v -refreshToken");

  return res.status(200).json({
    success: true,
    data: admins,
    message: admins.length === 0 ? "No admins found" : undefined,
  });
});
...
//teacher management
// Add these to staff.controller.js

const getTeachers = asyncHandler(async (req, res, next) => {
  const teachers = await Staff.find({ role: "teacher" }).sort({
    createdAt: -1,
  });
  return res.status(200).json({
    success: true,
    data: teachers,
    message: teachers.length === 0 ? "No teachers found" : undefined,
  });
});
...
const updateTeacher = asyncHandler(async (req, res, next) => {
  ...
  const teacher = await Staff.findOne({ _id, role: "teacher" });
  if (!teacher) {
    return res.status(404).json({
      success: false,
  ...
  const oldPhotoUrl = teacher.photoUrl;
  let uploadAvatar = null;
  ...
  if (uploadAvatar?.url) {
    teacher.photoUrl = uploadAvatar.url;
  }
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Error updating teacher record",
      error: error.message,
    });
  }
});
...
const deleteTeacher = asyncHandler(async (req, res, next) => {
  ...
  const deletedTeacher = await Staff.findByIdAndDelete(_id);
  if (!deletedTeacher) {
    return res.status(500).json({
      success: false,
